define(['phaser'], function (Phaser) {
    ...
    var Player = function (game, config) {
        ...
        this.state.thrust.available = this.state.thrust.maxAvailable;
    ...
    Player.prototype = {
        ...
        update: function () {
            ...
            if (this.game.engine.input.mousePointer.isDown) {
                ...
                var thrust = Math.min(this.game.engine.math.linear(this.state.thrust.minPower, this.state.thrust.maxPower, distance / this.state.maxDistance), this.state.thrust.available);

                this.body.force.x = Math.cos(angle) * thrust;
                this.body.force.y = Math.sin(angle) * thrust;

                this.state.thrust.available = Math.max(this.state.thrust.available - thrust, 0);

            } else {
                this.state.thrust.available = Math.min(this.state.thrust.available + (this.state.thrust.regenRate * this.state.thrust.maxAvailable), this.state.thrust.maxAvailable);
            }
